...
SAMPLE_SCENARIO_PATH = os.getenv(
    "VacAI_SCENARIO", "agents/vacAI/profiles/itinerary_empty_enhanced_default.json"
)
...
def _combined_before_agent_logic(callback_context: CallbackContext) -> CallbackContext:
    """
    Wrapper function to execute itinerary loading and guardrails sequentially.
    """
    # 1. Load the pre-created itinerary (First Callback)
    # The existing function: loads data and calls _set_initial_states
    _load_precreated_itinerary(callback_context)
    
    # 2. Run the guardrails on the newly loaded context (Second Callback)
    callback_context = governance.before_agent_callback(callback_context)
    
    # Return the final, fully initialized context
